...
- (void)initialization {
    ...
    if (_showProgressText) {
         ...
         *  设置时间戳
         */
//        self.progressLabel.text = [self addOTPWithTimeLag];
...
//        _progressLabel.font = [UIFont fontWithName:@"Zapfino" size:18];
...
- (void)setProgressLabelTextForTimeInterval{

//    self.progressLabel.text = [self addOTPWithTimeLag];
//    self.increaseFromLast = YES;
...
- (void)startAnimation {
    ...
    pathAnima.timingFunction = [CAMediaTimingFunction functionWithName:kCAMediaTimingFunctionLinear];
    //_increaseFromLast == YES?_lastProgress:0
    if (_lastProgress == 1.0) {
        _lastProgress = 0.0;
    }
